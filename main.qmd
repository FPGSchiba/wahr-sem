...
Angenomenn hier wurden einige Werte:

-   Wetter wahrscheinlichkeiten
    -   Gutes Wetter: `r round(wetter_probs[1],2)`
-   Im ersten Slot des Tages werden weniger leute sein
-   Essen Angenommene Werte (1. Slot, 2. Slot, 3. Slot) pro Tag
    -   Erwartungswert: `r essen_e`
    -   Standardabweichung: `r essen_sd`
-   Trinken Angenommene Werte (1. Slot, 2. Slot, 3. Slot) pro Tag
    -   Erwartungswert: `r trinken_e`
    -   Standardabweichung: `r trinken_sd`
-   Abfallgewicht Durschschnitt: `r format(abfall_gewicht_pro_einheit*1000000,scientific=FALSE)` g
...
2.  Simulationsschleife: Für nsim = `r format(nsim,scientific = FALSE)` Simulationen wird die Anzahl der Besucher und das Wetter für das Event zufällig bestimmt. Basierend auf diesen Informationen wird die Anzahl der Besucher pro Zeitslot berechnet.
...
::: {.column width="45%"}
Das Histogramm zeigt die Verteilung der Gesamtabfallproduktion über alle Simulationen hinweg. Der berechnete Mittelwert der Gesamtabfallproduktion liegt bei etwa `r round(calculated_mean,2)` Tonnen.
:::
:::
